...
const program_nodes = (
    ...
    variable = variable,
    tag = (ex) -> :(logic_leaf($(esc(ex)))),
    literal = literal,
    leaf = (ex) -> :(logic_leaf($(esc(ex)))),
    dimless = :(logic_leaf(dimless))
)
...
const instance_nodes = (
    ...
    tag = (ex) -> :($tag_instance($(variable_instance(ex)), $logic_leaf_instance($(esc(ex))))),
    literal = literal_instance,
    leaf = (ex) -> :($logic_leaf_instance($(esc(ex)))),
    dimless = :($logic_leaf_instance(dimless))
)
...
function display_expression(io, mime, node::Union{LogicNode, LogicNodeInstance})
...
function display_statement(io, mime, node::Union{LogicNode, LogicNodeInstance}, indent)
