...
@inline level_size(lvl::SparseBytemapLevel) = (level_size(lvl.lvl)..., lvl.I)
@inline level_axes(lvl::SparseBytemapLevel) = (level_axes(lvl.lvl)..., Base.OneTo(lvl.I))
...
function get_updater(fbr::VirtualDirtySubFiber{VirtualSparseBytemapLevel}, ctx, ::Union{Nothing, Extrude, Laminate}, protos...)
    (lvl, pos) = (fbr.lvl, fbr.pos)
    tag = lvl.ex
    Tp = lvl.Tp
    my_q = ctx.freshen(tag, :_q)
    ...
    Furlable(
        ...
        body = (ctx, idx, ext) -> AcceptSpike(
            ...
            tail = (ctx, idx) -> Thunk(
                ...
                body = get_updater(VirtualDirtySubFiber(lvl.lvl, value(my_q, lvl.Ti), dirty), ctx, protos...),
                epilogue = quote
                    if $dirty
                        $(fbr.dirty) = true
                        if !$(lvl.ex).tbl[$my_q]
