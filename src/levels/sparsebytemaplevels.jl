...
function get_updater(fbr::VirtualSubFiber{VirtualSparseBytemapLevel}, ctx, ::Union{Nothing, Extrude, Laminate}, protos...)
    (lvl, pos) = (fbr.lvl, fbr.pos)
    tag = lvl.ex
    Tp = lvl.Tp
    my_q = ctx.freshen(tag, :_q)
    ...
    Furlable(
        ...
        body = (ctx, idx, ext) -> AcceptSpike(
            ...
            tail = (ctx, idx) -> Thunk(
                ...
                body = get_updater(VirtualSubFiber(lvl.lvl, value(my_q, lvl.Ti)), ctx, protos...),
                epilogue = quote
