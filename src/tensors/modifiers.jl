...
function instantiate_reader(::VirtualPermit, ctx, proto_idx::typeof(defaultread))
    Furlable(
        body = nothing,
...
function instantiate_reader(::VirtualOffset, ctx, proto_delta::typeof(defaultread), proto_idx::typeof(defaultread))
    tns = Furlable(
        body = (ctx, ext) -> Lookup(
...
function instantiate_reader(arr::VirtualStaticOffset, ctx, proto_idx::typeof(defaultread))
    Furlable(
        body = nothing,
        fuse = (tns, ctx, ext) -> Pipeline([
            Phase(
...
function instantiate_reader(arr::VirtualWindow, ctx, proto_idx::typeof(defaultread))
    Furlable(
        body = nothing,
        fuse = (tns, ctx, ext) ->
