...
function instantiate(fbr::VirtualSubFiber{VirtualSparseTriangleLevel}, ctx, mode::Reader, protos)
    ...
    Thunk(
        ...
        body = (ctx) -> instantiate(SparseTriangleFollowTraversal(lvl, lvl.N, lvl.shape, lvl.N, value(q)), ctx, mode::Reader, protos)
    )
end
        ...
        Furlable(
            body = (ctx, ext) -> Sequence([
                Phase(
                    stop = (ctx, ext) -> j,
                    body = (ctx, ext) -> Lookup(
                        body = (ctx, i) -> instantiate(VirtualSubFiber(lvl.lvl, call(+, q, -1, i)), ctx, mode::Reader, subprotos)
                    )
                ),
                Phase(
                ...
                Phase(
                    ...
                    body = (ctx, ext) -> Lookup(
                        body = (ctx, i) -> Thunk(
                            ...
                            body = (ctx) -> instantiate(SparseTriangleFollowTraversal(lvl, d - 1, i, n, value(s)), ctx, mode::Reader, subprotos)
                        )
                    )
                ),
                Phase(
