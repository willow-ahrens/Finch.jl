...
function instantiate(fbr::VirtualSparseSubFiber{VirtualSparseTriangleLevel}, ctx, mode::Updater, protos)
    (lvl, pos) = (fbr.lvl, fbr.pos)
    tag = lvl.ex
    Ti = lvl.Ti
        ...
        Furlable(
            body = (ctx, ext) -> Sequence([
                Phase(
                    stop = (ctx, ext) -> j,
                    body = (ctx, ext) -> Lookup(
                        body = (ctx, i) -> instantiate(VirtualSparseSubFiber(lvl.lvl, call(+, q, -1, i), dirty), ctx, mode, subprotos)
                    )
                ),
                Phase(
