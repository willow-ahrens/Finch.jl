...
function unfurl_reader(fbr::VirtualSubFiber{VirtualElementLevel}, ctx)
...
function unfurl_updater(fbr::VirtualSubFiber{VirtualElementLevel}, ctx)
    (lvl, pos) = (fbr.lvl, fbr.pos)
    VirtualScalar(nothing, lvl.Tv, lvl.D, gensym(), :($(lvl.ex).val[$(ctx(pos))]))
end

function unfurl_updater(fbr::VirtualTrackedSubFiber{VirtualElementLevel}, ctx)
    (lvl, pos) = (fbr.lvl, fbr.pos)
