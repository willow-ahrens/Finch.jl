...
function instantiate_reader(trv::SparseCOOWalkTraversal, ctx, ::Union{typeof(defaultread), typeof(walk)}, protos...)
                        ...
                        body = if R == 1
                            ...
                            Thunk(
                                ...
                                body = (ctx) -> Step(
                                    ...
                                    chunk = Spike(
                                        body = Fill(virtual_level_default(lvl)),
                                        tail = instantiate_reader(SparseCOOWalkTraversal(lvl, R - 1, value(my_q, lvl.Ti), value(my_q_step, lvl.Ti)), ctx, protos...),
                                    ),
                                    next = (ctx, ext) -> quote
                                        $my_q = $my_q_step
                                    end
...
function instantiate_updater(trv::SparseCOOExtrudeTraversal, ctx, ::Union{typeof(defaultupdate), typeof(extrude)}, protos...)
    ...
    Furlable(
        ...
        body = (ctx, ext) -> 
            if length(coords) + 1 < lvl.N
                Lookup(
                    body = (ctx, i) -> instantiate_updater(SparseCOOExtrudeTraversal(lvl, qos, fbr_dirty, (i, coords...)), ctx, protos...),
                )
            else
