...
mutable struct VirtualRootSparseListLevel <: AbstractVirtualLevel
    ...
    idx
    shape
    qos_fill # next position to use
    qos_stop # max position avaliable
    endpoint # endpoint.
end
...
function virtualize(ex, ::Type{RootSparseListLevel{Ti, Idx, Lvl}}, ctx, tag=:lvl) where {Ti, Idx, Lvl}
    ...
    endpoint = value(:($sym.endpoint[]), Ti)
    qos_fill = freshen(ctx, sym, :_qos_fill)
    qos_stop = freshen(ctx, sym, :_qos_stop)
    VirtualRootSparseListLevel(lvl_2, sym, Ti, idx, shape, qos_fill, qos_stop, endpoint)
end
