...
function unfurl_reader(fbr::VirtualSubFiber{VirtualRepeatRLELevel}, ctx, ::Union{typeof(defaultread), typeof(walk)})
...
unfurl_updater(fbr::VirtualSubFiber{VirtualRepeatRLELevel}, ctx, protos...) = 
    unfurl_updater(VirtualTrackedSubFiber(fbr.lvl, fbr.pos, ctx.freshen(:null)), ctx, protos...)
function unfurl_updater(fbr::VirtualTrackedSubFiber{VirtualRepeatRLELevel}, ctx, ::Union{typeof(defaultupdate), typeof(extrude)})
    (lvl, pos) = (fbr.lvl, fbr.pos)
    tag = lvl.ex
    Tp = lvl.Tp
    Ti = lvl.Ti
