@testset "permit" begin
    ...
    println(@index_code_lowered @loop i C[i] += coalesce(A[permit[i]], B[permit[$(Finch.StaticOffset(shift=5))[i]]], 0)) #TODO get rid of + and 0 and also add in the right offset syntax
    #@index @loop i C[i] += coalesce(A[permit[i]], B[permit[offset[5, i]]], 0)
    @index @loop i C[i] += coalesce(A[permit[i]], B[permit[$(Finch.StaticOffset(shift=5))[i]]], 0)
    println(FiberArray(C))
