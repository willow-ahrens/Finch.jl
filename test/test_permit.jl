@testset "permit" begin
    ...
    println(@index_code_lowered @loop i C[i] = coalesce(A[permit[i]], B[$(Finch.StaticOffset(shift=5))[permit[i]]]))
    #@index @loop i C[i] += coalesce(A[permit[i]], B[permit[offset[5, i]]], 0)
    @index @loop i C[i] = coalesce(A[permit[i]], B[$(Finch.StaticOffset(shift=5))[permit[i]]])
    println(FiberArray(C))
