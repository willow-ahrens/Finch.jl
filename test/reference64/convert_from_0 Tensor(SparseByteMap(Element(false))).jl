begin
    ...
    tmp_lvl_srt = ((ex.bodies[1]).bodies[2]).body.rhs.tns.bind.lvl.srt
    ...
    if tmp_lvl_r != 0 && tmp_lvl_r < tmp_lvl_r_stop
        tmp_lvl_i_stop = last(tmp_lvl_srt[tmp_lvl_r_stop - 1])
    else
        tmp_lvl_i_stop = 0
    end
    ...
    result = something(nothing, (res = Tensor((SparseListLevel){Int64}(res_lvl_2, tmp_lvl.shape, res_lvl_ptr, res_lvl_idx)),))
