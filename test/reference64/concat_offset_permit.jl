begin
    ...
    if phase_stop >= -9
        i = -9
        if B_lvl.idx[B_lvl_q] < -9 + +10
            B_lvl_q = scansearch(B_lvl.idx, -9 + +10, B_lvl_q, B_lvl_q_stop - 1)
        end
        while i <= phase_stop
    ...
    phase_start_7 = max(-9, 1 + A_lvl.shape)
    ...
    phase_start_10 = max(1, -9, -9 + B_lvl_i1)
