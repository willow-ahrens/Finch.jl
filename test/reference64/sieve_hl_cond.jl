begin
    ...
    phase_start = max(1, 1 + (1 - 1))
    phase_stop_2 = min(A_lvl_i1, A_lvl.shape, 1)
    if phase_stop_2 >= phase_start
        if A_lvl_idx[A_lvl_q] < phase_start
            A_lvl_q = Finch.scansearch(A_lvl_idx, phase_start, A_lvl_q, A_lvl_q_stop - 1)
        end
        while true
