@testset "continuous" begin
    ...
    let
        ...
        @test check_println("continuous/continuous_pinpoint_sl.txt", String(take!(io)))

        @repl io @finch_code (z2 .= 0; for i=_; z2[] += y[i] end)
        @repl io @finch (z2 .= 0; for i=_; z2[] += y[i] end)
        @test check_println("continuous/continuous_pinpoint_rle.txt", String(take!(io)))
    ...
    let
        ...
        @test check_println("continuous/continuous_intersect.txt", String(take!(io)))

        @repl io @finch_code (z .= 0; for i=_; z[i] += x[i] + y[i]  end)
        @repl io @finch (z .= 0; for i=_; z[i] += x[i] + y[i]  end)
        @test check_println("continuous/continuous_union.txt", String(take!(io)))

        @repl io @finch_code (s .= 0; for i=_; s[] += (x[i] * y[i])  end)
        @repl io @finch (s .= 0; for i=_; s[] += (x[i] * y[i])  end)
        @test check_println("continuous/continuous_intersect_counting.txt", String(take!(io)))

        @repl io @finch_code (s .= 0; for i=_; s[] += (x[i] * y[i]) * d(i) end)
        @repl io @finch (s .= 0; for i=_; s[] += (x[i] * y[i]) * d(i)  end)
        @test check_println("continuous/continuous_intersect_lebesgue.txt", String(take!(io)))
        
        @repl io @finch_code (s .= 0; for i=_; s[] += (x[i] + y[i])  end)
        @repl io @finch (s .= 0; for i=_; s[] += (x[i] + y[i])  end)
        @test check_println("continuous/continuous_union_counting.txt", String(take!(io)))

        @repl io @finch_code (s .= 0; for i=_; s[] += (x[i] + y[i]) * d(i) end)
        @repl io @finch (s .= 0; for i=_; s[] += (x[i] + y[i]) * d(i)  end)
        @test check_println("continuous/continuous_union_lebesgue.txt", String(take!(io)))
    end
    ...
    let
        ...
        @test check_println("continuous/continuous_2d_intersect.txt", String(take!(io)))  

        @repl io @finch_code (z .= 0; for i=_, j=_; z[j,i] += x[j,i] + y[j,i] end)
        @repl io @finch (z .= 0; for i=_, j=_; z[j,i] += x[j,i] + y[j,i] end)
        @test check_println("continuous/continuous_2d_union.txt", String(take!(io)))  

        @repl io @finch_code (s .= 0; for i=_, j=_; s[] += (x[j,i] * y[j,i]) * d(i,j) end)
        @repl io @finch (s .= 0; for i=_, j=_; s[] += (x[j,i] * y[j,i]) * d(i,j) end)
        @test check_println("continuous/continuous_2d_intersect_lebesgue.txt", String(take!(io)))  

        @repl io @finch_code (s .= 0; for i=_, j=_; s[] += (x[j,i] * y[j,i]) end)
        @repl io @finch (s .= 0; for i=_, j=_; s[] += (x[j,i] * y[j,i]) end)
        @test check_println("continuous/continuous_2d_intersect_counting.txt", String(take!(io)))  

        @repl io @finch_code (s .= 0; for i=_, j=_; s[] += (x[j,i] + y[j,i]) end)
        @repl io @finch (s .= 0; for i=_, j=_; s[] += (x[j,i] + y[j,i]) end)
        @test check_println("continuous/continuous_2d_union_counting.txt", String(take!(io))) 

        @repl io @finch_code (s .= 0; for i=_, j=_; s[] += (x[j,i] + y[j,i]) * d(i,j) end)
        @repl io @finch (s .= 0; for i=_, j=_; s[] += (x[j,i] + y[j,i]) * d(i,j) end)
        @test check_println("continuous/continuous_2d_union_lebesgue.txt", String(take!(io)))  
    ...
    let
        ...
        @test check_println("continuous/continuous_2d_itvl_sum_sl.txt", String(take!(io)))  

        @repl io @finch_code (s2 .= 0; for i=_, j=_; s2[] += x1[j,i] * y[j] * d(j) end)
        @repl io @finch (s2 .= 0; for i=_, j=_; s2[] += x1[j,i] * y[j] * d(j) end)
        @test check_println("continuous/continuous_2d_itvl_sum_rle.txt", String(take!(io)))  
