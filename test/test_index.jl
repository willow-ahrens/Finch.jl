@testset "index" begin
    ...
    @test check_println("index/sieve_hl_cond.jl", String(@finch_code (B .= 0; for j=_; if j == 1 B[] += A[j] end end)))
    ...
    @test check_println("index/sieve_hl_select.jl", @finch_code (for j=_; if diagmask[3, j] B[] += A[j] end end))
    ...
    @test check_println("index/gather_hl.jl", @finch_code (B[] += A[5]))
    ...
    @test check_println("index/concat_offset_permit.jl", @finch_code (C .= 0; for i=_; C[i] = coalesce(A[~i], B[~(i - 10)]) end))
    @finch (C .= 0; for i=_; C[i] = coalesce(A[~i], B[~(i - 10)]) end)
    @test reference_isequal(C, C_ref)

    F = fiber(Int64[1,1,1,1,1])

    @test check_println("index/sparse_conv.jl", @finch_code (C .= 0; for i=_, j=_; C[i] += (A[i] != 0) * coalesce(A[j - i + 3], 0) * F[j] end))
    ...
    @test check_println("index/sparse_window.jl", @finch_code (C .= 0; for i=_; C[i] = A[(2:4)[i]] end))
    ...
    let
        ...
        @test check_println("index/chunkmask.txt", String(take!(io)))
    end
    ...
    let
        ...
        @test check_println("index/swizzle_1.txt", @finch_code begin
            for i = _
                for j = _
                    for k = _
        ...
        @test check_println("index/swizzle_2.txt", @finch_code begin
            for i = _
                for j = _
                    for k = _
                        x[] += A[k, j, i]
