begin
    ...
    if tmp_lvl.shape >= 1
        for j_5 = 1:tmp_lvl.shape
            ...
            phase_stop = min(tmp_lvl.shape, j_5)
            if phase_stop >= 1
    ...
    (res = Fiber((SparseListLevel){Int32}((SparseListLevel){Int32}(res_lvl_3, tmp_lvl.shape, res_lvl_ptr_2, res_lvl_idx_2), tmp_lvl.shape, res_lvl_ptr, res_lvl_idx)),)
end
