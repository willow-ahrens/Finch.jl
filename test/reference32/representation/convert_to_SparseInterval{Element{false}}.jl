quote
    ...
    tmp_lvl_qos = 0 + 1
    tmp_lvl_ptr[1 + 1] == 0 || throw(FinchProtocolError("SparseIntervalLevels can only be updated once"))
    ...
    if phase_stop >= 1
        ...
        while true
            ...
            if ref_lvl_i < phase_stop
                ...
                tmp_lvl_qos == 0 + 1 || throw(FinchProtocolError("SparseIntervalLevels can only be updated once"))
    ...
    result = (tmp = Tensor((SparseIntervalLevel){Int32}(tmp_lvl_2, ref_lvl.shape, tmp_lvl.ptr, tmp_lvl.left, tmp_lvl.right)),)
    result
end
