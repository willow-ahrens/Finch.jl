quote
    ...
    tmp_lvl_qos = 0 + 1
    tmp_lvl_ptr[1 + 1] == 0 || throw(FinchProtocolError("SparsePointLevels can only be updated once"))
    0 < 1 || throw(FinchProtocolError("SparsePointLevels cannot be updated multiple times"))
    ...
    if phase_stop >= 1
        ...
        while true
            ...
            if ref_lvl_i < phase_stop
                ...
                tmp_lvl_qos == 0 + 1 || throw(FinchProtocolError("SparsePointLevels can only be updated once"))
    ...
    result = (tmp = Tensor((SparsePointLevel){Int32}(tmp_lvl_2, ref_lvl.shape, tmp_lvl_ptr, tmp_lvl_idx)),)
    result
end
