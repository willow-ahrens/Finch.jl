begin
    ...
    phase_start_2 = max(1, i_start)
    phase_stop_2 = min((((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta, A_lvl.shape, i_stop)
    if phase_stop_2 >= phase_start_2
        ...
        if phase_stop_3 >= phase_start_2
            ...
            if A_lvl.idx[A_lvl_q] < phase_start_2
                A_lvl_q = scansearch(A_lvl.idx, phase_start_2, A_lvl_q, A_lvl_q_stop - 1)
            end
    ...
    phase_start_6 = max(i_start, 1 + A_lvl.shape)
    phase_stop_6 = min((((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta, i_stop)
    ...
    phase_start_7 = max(1 + (((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta, i_start)
    phase_stop_7 = min((((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta + B_lvl.shape, i_stop, 0)
    if phase_stop_7 >= phase_start_7
        ...
        if phase_stop_8 >= phase_start_7
            ...
            if B_lvl.idx[B_lvl_q] < phase_start_7 + -((((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta)
                B_lvl_q = scansearch(B_lvl.idx, phase_start_7 + -((((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta), B_lvl_q, B_lvl_q_stop - 1)
            end
            while i <= phase_stop_8
                B_lvl_i = B_lvl.idx[B_lvl_q]
                phase_stop_9 = min(phase_stop_8, (((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta + B_lvl_i)
                if B_lvl_i == phase_stop_9 + -((((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta)
    ...
    if phase_stop_11 >= phase_start_11
        ...
        if phase_stop_12 >= phase_start_11
            ...
            while i <= phase_stop_12
                A_lvl_i = A_lvl.idx[A_lvl_q]
                B_lvl_i = B_lvl.idx[B_lvl_q]
                phase_stop_13 = min(A_lvl_i, (((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta + B_lvl_i, phase_stop_12)
                if phase_stop_13 >= i
        ...
        phase_start_15 = max(1 + (((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta + B_lvl_i1, phase_start_11)
        phase_stop_15 = min(A_lvl_i1, phase_stop_11)
        if phase_stop_15 >= phase_start_15
            i = phase_start_15
            if A_lvl.idx[A_lvl_q] < phase_start_15
                A_lvl_q = scansearch(A_lvl.idx, phase_start_15, A_lvl_q, A_lvl_q_stop - 1)
            end
    ...
    phase_start_18 = max(1 + (((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta, i_start, 1 + A_lvl.shape)
    phase_stop_18 = min((((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta + B_lvl.shape, i_stop)
    if phase_stop_18 >= phase_start_18
        ...
        phase_stop_19 = min((((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta + B_lvl_i1, phase_stop_18)
        if phase_stop_19 >= phase_start_18
            i = phase_start_18
            if B_lvl.idx[B_lvl_q] < phase_start_18 + -((((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta)
                B_lvl_q = scansearch(B_lvl.idx, phase_start_18 + -((((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta), B_lvl_q, B_lvl_q_stop - 1)
            end
            while i <= phase_stop_19
                B_lvl_i = B_lvl.idx[B_lvl_q]
                phase_stop_20 = min((((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta + B_lvl_i, phase_stop_19)
                if B_lvl_i == phase_stop_20 + -((((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta)
    ...
    phase_start_22 = max(i_start, 1 + (((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta + B_lvl.shape)
    phase_stop_22 = min(i_stop, 0)
    ...
    phase_start_23 = max(1, i_start, 1 + (((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta + B_lvl.shape)
    phase_stop_23 = min(A_lvl.shape, i_stop)
    if phase_stop_23 >= phase_start_23
        ...
        if phase_stop_24 >= phase_start_23
            ...
            if A_lvl.idx[A_lvl_q] < phase_start_23
                A_lvl_q = scansearch(A_lvl.idx, phase_start_23, A_lvl_q, A_lvl_q_stop - 1)
            end
    ...
    phase_start_27 = max(i_start, 1 + A_lvl.shape, 1 + (((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta + B_lvl.shape)
    ...
    (C = Fiber((SparseListLevel){Int64, Int32}(C_lvl_2, max(A_lvl.shape, B_lvl.shape + (((ex.bodies[2]).body.rhs.args[2]).idxs[1]).tns.tns.delta), C_lvl.ptr, C_lvl.idx)),)
end
