begin
    ...
    if phase_stop >= 1
        ...
        if A_lvl.idx[A_lvl_q] < 1 + -((1 - first(win)))
            A_lvl_q = Finch.scansearch(A_lvl.idx, 1 + -((1 - first(win))), A_lvl_q, A_lvl_q_stop - 1)
        end
        while i <= phase_stop
    ...
    (C = Fiber((SparseListLevel){Int64, Int32}(C_lvl_2, last(win) + (1 - first(win)), C_lvl.ptr, C_lvl.idx)),)
end
